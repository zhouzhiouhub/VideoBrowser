...
- 默认显示 App 自定义首页，提交关键词后按当前默认搜索引擎打开搜索结果页。
- 对常见广告请求进行本地规则拦截。
- 在页面加载完成后注入应用内置脚本，隐藏广告、弹窗、登录提示等干扰元素，并增强网页视频体验。
- 对直接视频地址或本地视频文件使用 AndroidX Media3 ExoPlayer 原生播放。
- 通过功能中心管理书签、历史、下载、本地文件、站点设置、浏览器设置、拦截日志、白名单、手动屏蔽规则、站点权限记录和关于信息。
- 书签和历史记录点击可直接打开；书签长按可编辑标题、新标签打开、复制、分享和移除，历史记录按日期分组显示，长按进入选择模式后可全选、删除或完成选择。收藏夹支持文件夹分组、导入和导出，浏览历史支持按时间范围清空。

项目没有后端服务，也没有外部数据库。设置、收藏、历史、下载记录、白名单、自定义快捷入口、自定义搜索引擎等数据主要保存在本机 `SharedPreferences` 中。
...
- 主入口是 `MainActivity`。
- 使用 WebView 加载网页。
- 支持地址栏输入 URL 或搜索关键词；内部 `about:` 地址只接受 `about:blank`。
- 首页保留 App 自己的地址/搜索入口，不自动加载百度、360、搜狗、夸克、UC 或 Bing 等搜索引擎首页。
- 支持前进、后退、刷新、页面进度、页面标题同步，加载中可停止加载。
- 地址栏显示当前页面 HTTPS/HTTP 安全状态，点击图标可查看站点连接、证书验证边界和混合内容策略。
- 支持系统可用时启用 WebView Safe Browsing，命中风险页面时退回安全页。
- 默认禁止 `file://` 页面跨文件或跨来源访问本地资源。
- 支持复制链接、分享页面、保存网页为 MHTML 离线归档并从本地文件入口重新打开、打印或另存为 PDF、收藏当前页。
- 支持长按网页链接或图片后新标签打开、下载、复制或分享。
- 支持 HTTP Basic Auth 站点登录弹窗，HTTP Basic Auth 只保留一个待处理认证弹窗。
- 支持网站请求客户端证书时调用 Android 系统证书选择器。
- 支持网页 JavaScript alert、confirm、prompt 和离页确认弹窗。
- 网页跳转到外部应用协议时会在应用内静默拦截，不启动其他应用；`intent://` 链接可按网页提供的 fallback 地址回退加载。
- 主页面加载失败时显示本地错误页，并提供原地址重试入口；页面工具可直接回到 App 自定义首页。
- 保留主页地址设置用于兼容和历史过滤；App 启动、主页按钮和空白新标签页不再自动加载该地址。
- 支持从功能中心方格进入默认搜索引擎页面，并可添加自定义搜索引擎。
- 标签页管理支持新建、恢复最近关闭、复制、切换、关闭、关闭其他标签、关闭全部标签、复制标签链接和分享标签链接，网页弹窗标签可响应关闭请求，非用户手势触发的新窗口请求会被拒绝。标准标签页会话只恢复带主机名的 HTTP/HTTPS 页面 URL。
- 支持桌面模式：切换 User-Agent、开启 wide viewport，并重新加载页面。
- 支持双指缩放网页，并隐藏系统缩放悬浮按钮。
- 支持顶部和底部浏览器控件随页面滚动隐藏。
- 支持在浏览器设置中允许或阻止第三方 Cookie。
- 默认阻止 HTTPS 页面混合内容，并可在浏览器设置中调整。
- HTTPS 页面跳转到 HTTP 明文页面前会先确认，避免静默降级到未加密页面。
- 支持在浏览器设置中调整网页文字大小。
- 网页请求相机、麦克风或定位时，除 Android 运行时权限外，还会按站点确认永久允许、仅本次允许或阻止；只会授予相机和麦克风资源，未知网页权限会被拒绝；当前站点设置中可改回每次询问，站点权限记录页可集中查看和清理已保存决策。
...
默认搜索引擎 ID 是 `baidu`，用户在功能中心切换默认搜索引擎后会写入设置，重新打开 App 后继续使用上次选择。App 启动首页是自定义首页，不自动加载搜索引擎首页。搜索引擎的 `homeUrl` 仍保留在模型中，用于兼容设置和历史过滤。功能中心的搜索引擎页面会展示内置搜索引擎和用户添加的自定义搜索引擎；自定义项保存名称和搜索 URL 前缀，地址栏会在前缀后追加编码后的关键词。长按搜索引擎可移除该项，内置搜索引擎会保存为隐藏状态，恢复默认设置后重新显示；列表至少保留一个搜索引擎。
...
- 下载器只接受 HTTP/HTTPS 下载地址。
- 下载文件名会移除路径分隔符、控制字符和常见非法字符。
...
- 重命名文件。
- 删除文件。
- MHTML/MHT 网页归档会在浏览器中打开，用于离线查看保存的网页。
...
- 使用 `LOAD_NO_CACHE`。
- 不写入历史记录。
- 新的相机、麦克风和定位站点权限选择不会写入持久记录，仅保留本次会话临时允许。
...
| `browser/` | WebView 管理、页面状态、请求上下文、地址栏、外部协议拦截 |
...
- 当前 App 自定义首页不再展示自定义快捷入口；相关存储保留用于兼容后续功能。
...
| --- | --- |
| 搜索引擎 | `baidu` |
| 首页 URL（不自动加载） | `https://m.baidu.com/` |
| 默认视频倍速 | `1f` |
| 网页文字大小 | `100%` |
| 视频总是从头播放 | 关闭 |
| 广告请求拦截 | 开启 |
...
“恢复默认设置”会清理设置、站点例外、用户白名单、用户手动屏蔽规则、自定义快捷入口、自定义搜索引擎、收藏、历史、缓存、Cookie 和站点数据，并重新创建 Activity。
