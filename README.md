...
默认搜索引擎 ID 是 `baidu`，用户切换搜索引擎只影响当前运行会话，不写入设置。App 启动首页是自定义首页，不自动加载搜索引擎首页。搜索引擎的 `homeUrl` 仍保留在模型中，用于兼容设置和历史过滤。功能中心的搜索引擎页面会展示内置搜索引擎和用户添加的自定义搜索引擎；自定义项保存名称和搜索 URL 前缀，地址栏会在前缀后追加编码后的关键词。
