...
- 书签和历史记录点击可直接打开；书签长按可编辑标题、新标签打开、复制、分享和移除，历史记录按日期分组显示，长按会标注选中记录并只保留复制链接和移除。收藏夹支持文件夹分组、导入和导出，浏览历史支持按时间范围清空。
...
默认搜索引擎 ID 是 `baidu`，用户切换搜索引擎只影响当前运行会话，不写入设置。App 启动首页是自定义首页，不自动加载搜索引擎首页。搜索引擎的 `homeUrl` 仍保留在模型中，用于兼容设置和历史过滤。功能中心的搜索引擎页面会展示内置搜索引擎和用户添加的自定义搜索引擎；自定义项保存名称和搜索 URL 前缀，地址栏会在前缀后追加编码后的关键词。长按搜索引擎可移除该项，内置搜索引擎会保存为隐藏状态，恢复默认设置后重新显示；列表至少保留一个搜索引擎。
