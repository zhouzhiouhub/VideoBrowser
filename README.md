...
- 首页会从浏览历史生成最近访问入口，并排除搜索引擎主页和已固定的快捷入口；最近访问入口可长按移除。
- 支持前进、后退、刷新、页面进度、页面标题同步，加载中可停止加载，页面工具可执行前进。
- 地址栏显示当前页面 HTTPS/HTTP 安全状态，点击图标可查看站点连接、证书验证边界和混合内容策略。
- 支持系统可用时启用 WebView Safe Browsing，命中风险页面时退回安全页。
- 默认禁止 `file://` 页面跨文件或跨来源访问本地资源。
- 支持页内查找，并可跳转上一处或下一处匹配，并显示匹配数量。
- 支持系统浏览器打开、复制链接、分享页面、保存网页为 MHTML 离线归档并从本地文件入口重新打开、打印或另存为 PDF、收藏当前页；系统浏览器打开仅处理带主机名的 HTTP/HTTPS 页面 URL。
- 支持长按网页链接或图片后新标签打开、下载、复制、分享或系统浏览器打开。
...
- 网页跳转到外部应用协议时会直接交给系统处理；无可用应用时显示提示，`intent://` 链接可按网页提供的 fallback 地址回退加载。
- 主页面加载失败时显示本地错误页，并提供原地址重试入口；页面工具可直接回到已配置主页。
- 支持在浏览器设置中修改主页地址，也可将当前页面设为主页。
- 支持在浏览器设置中修改默认搜索引擎。
