...
本文件是 VideoBrowser 仓库的项目级 Codex 指令。处理本仓库任务时必须优先遵守这里的规则；`skills.md` 可作为说明文档，但强制约束以本文件为准。

## 先审查，再修改

每次修改前必须先搜索现有实现、调用点和契约测试。优先检查：
...
如果目标能力已经存在，禁止再次实现一套。只能直接复用、扩展原模块，或抽出共享模块后迁移调用方并移除重复代码。

## 复用优先

能复用的代码只保留一份。发现两个或更多调用点实现同一类能力时，必须优先收敛到职责最匹配的已有模块。
...
- 新建宽泛的 `Common`、`Helper`、`Manager` 文件来收纳无关逻辑。
...
适合使用 `projectFile(...).readText()` 的契约测试锁定模块归属，也可以补充普通单元测试验证行为。
