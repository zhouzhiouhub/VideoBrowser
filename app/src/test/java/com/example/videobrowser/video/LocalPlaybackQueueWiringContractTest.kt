...
class LocalPlaybackQueueWiringContractTest {
    ...
    fun playerActivityRestoresQueueFromIntentAndExposesQueueControls() {
        ...
        assertTrue(playerActivity.contains("onPreviousMediaRequested = ::playPreviousMedia"))
        assertTrue(playerActivity.contains("onNextMediaRequested = ::playNextMedia"))
        assertTrue(playerActivity.contains("onRepeatModeRequested = ::cycleRepeatMode"))
        assertTrue(playerActivity.contains("Player.REPEAT_MODE_ONE"))
        assertTrue(playerActivity.contains("STATE_REPEAT_MODE"))
        assertTrue(playerActivity.contains("outState.putString(STATE_REPEAT_MODE, repeatMode.name)"))
